...
#define ITEM_LEN_MAX 536870912 // = 512Mb
...
protected:
    ...
    bool _validate_clrf(const std::string& bstr);
    bool _cache_has_valid_item(std::string& item);
    void _update_cache();

    // Read up to n_bytes from fd

    std::string read_from_fd(int n_bytes);

    // Retruns next item on _read_fd
...
public:
    ...
    // Returns the request as a vector of string with first element being a command name
