...
// If there is a valid (CRLF terminated) item in cache, populate 'item' with it and return true
...
bool RESPParser::_cache_has_valid_item(std::string &item)
{
    for(int i = 0; i < _read_cache.length(); i++){
        item += -_read_cache[i]; // this is probably bad as adding char by char
...
// size_item must look like: "*<number-of-elements>\r\n"
...
// size_item must look like: "$<length>\r\n"
...
// Return true if bstr is terminated with CRLF
bool RESPParser::_validate_clrf(const std::string &bstr){
    int len = bstr.length();
...
std::vector<std::string> RESPParser::read_new_request(){
    ...
    for(int i = 0; i < size; ++i){
        ...
        if(!_validate_array_size(bstr_size_item)){
