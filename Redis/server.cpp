...
void periodic_snapshot(){
    ...
    while(true){
        redis_save({"save"});
        std::this_thread::sleep_for(std::chrono::minutes(redis::GlobalConfig.snapshot_period));
    }
...
void process_request(const std::vector<std::string>& req, int client_fd){
    ...
    if(req[0] == "COMMAND"){
        ...
        if(output == nullptr){
            throw RedisServerError("Redis cmd failed to retunr a valid RObject!");
        }
...
void handle_client(int client_fd){
    ...
    while(true){
        ...
        try{
            ...
            if(req.size() == 0){
                throw RedisServerError("Read empty quest!");
            }
...
int setup_server(){
    ...
    if((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1){
        // perror append errno to your msg
    ...
    if(setsockopt(server_fd, SQL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) == -1){
        die("setcokopt");
    }
