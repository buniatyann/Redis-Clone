...
public:
    ...
    // get a value of key from data map, return false if key doesn't exists or is expired.
    // If a key is expired, it is also removed

    bool get(const std::string& key, std::string& value) const;

    // Checked if there is an entry for a key in RedisStore (chekc both data and list_data maps)
    ...
    // Decrements if revese = true

    int incr(const std::string key, bool reverse = false);

    // Push the vals from left, if reverse = true, then push from right
...
private:
    ...
    // mutex to syncronize access to _Instance
